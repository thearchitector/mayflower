...
export class PyClass {
  ...
  async #init(bridge = globalThis.__pythonBridge) {
    ...
    const [ffid, pyClass] = await bridge.makePyClass(this, name, {
      ...
      bases: this.#superclass
        ? [[sup.ffid, this.#superargs, this.#superkwargs]]
        : [],
    });
    this.pyffid = ffid;
    ...
    const makeProxy = (target, forceParent) => {
      return new Proxy(target, {
        ...
        set: (target, prop, val) => {
          const pname = prop;
          if (prop === "parent")
            throw RangeError("illegal reserved property change");
...
export class Bridge {
  ...
  async call(ffid, stack, args, kwargs, set, timeout) {
    ...
    const resp = await waitFor(
      (resolve) =>
        this.com.writeRaw(payload, r, (pre) => {
          if (pre.key === "pre") {
    ...
    logDebug("call", ffid, stack, args, resp);
    switch (resp.key) {
      case "string":
      case "int":
        return resp.val; // Primitives don't need wrapping
  ...
  makePyObject(ffid, inspectString) {
    ...
    const handler = {
      get: (target, prop, receiver) => {
        ...
        logDebug("`prop", next.callstack, prop);
        if (prop === "$$") return target;
        if (prop === "ffid") return ffid;
        if (prop === "toJSON") return () => ({ ffid });
        if (prop === "toString" && inspectString) return () => inspectString;
        if (prop === "then") {
          // Avoid .then loops
          ...
          return (resolve, reject) => {
            const suppressErrors =
              next.callstack[next.callstack.length - 1].endsWith?.("$");
            if (suppressErrors) {
              next.callstack.push(next.callstack.pop().replace("$", ""));
            }
            this.get(ffid, next.callstack, [], suppressErrors)
              .then(resolve)
              .catch(reject);
        ...
        if (typeof prop === "symbol") {
          ...
          logDebug("Get symbol", next.callstack, prop);
          return;
        }
    ...
    return new Proxy(new Dummy(), handler);
  }
}
