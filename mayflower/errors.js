...
function formatLine(line) {
  ...
  for (const second of secondary)
    line = line.replace(new RegExp(second, "g"), chalk.blueBright(second) + "");
  return line;
}
...
function printError(
  ...
  log(
    "*** PY ***",
    chalk.white.bgRedBright.bold(" Python Error "),
    `Call to '${failedCall.replace("~~", "")}' failed:`
  );
    ...
    if (!(line.includes("pythonia") && !allowInternal) && !foundMainLine) {
      ...
      if (path) {
        ...
        const file = fs.readFileSync(
          fpath.startsWith("file:") ? new URL(fpath) : fpath,
          "utf-8"
        );
...
export function getErrorMessage(failedCall, jsStacktrace, pyStacktrace) {
  console.log(jsStacktrace, pyStacktrace);
  return "";
  // try {
  //   const [jse, jss] =
