...
export class JSBridge {
  ...
  async get(r, ffid, attr) {
    ...
    switch (type) {
      case "string":
        return this.ipc.send({ r, key: "string", val: v });
      case "big":
        return this.ipc.send({ r, key: "big", val: Number(v) });
      case "num":
        return this.ipc.send({ r, key: "num", val: v });
      case "py":
        return this.ipc.send({ r, key: "py", val: v.ffid });
      case "class":
        this.m[++this.ffid] = v;
  ...
  async inspect(r, ffid, mode) {
    const colors = false; // supportsColors && mode === "str";
    const s = (await this.m[ffid], { colors }).toString();
